...
renderValueBox({
  mm_name <- mm_ify()$name
  valueBox(
    value = paste0("Participations is ", basicFormatter(mm_ify()$rr,"Pct",1), " as risky as ", mm_ify()$name),
    color = c("#8e8db0")
  )
})
```
...
### Probability of Individual Death {.value-box}
...
renderValueBox({
  ...
  valueBox(
    value = Odds(),
    caption = paste0("95% CI: ", Odds_low(), " to ", Odds_high()),
    color = c("#45497d")
  )
})
```
...
renderPlot({
  mm_tribble %>% 
      add_row(activity="HCT Volunteer (Male)", 
          micromorts=50, col="Participate") %>% 
  add_row(activity="HCT Volunteer (Female)", 
          micromorts=60, col="Participate") %>% 
...
Long Term Risks
=======================================================================
<div class="LTR" width='100%' height='600px'>
