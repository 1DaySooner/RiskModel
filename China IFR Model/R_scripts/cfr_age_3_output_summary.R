...
# Detailed output, modificed for HCT:


m_thin_df= data.frame(t(apply(m_thin, 2,  summarise_mcmc_single, q =c((0:100)/100) )))


detailed_posterior_summary <- summarise_mcmc(m_thin, q = c((0:100)/100))
...
# Needed for the below analysis of IFR / Hospitalization by age 
...
ifr_age <- predicted %>%
  ...
  summarise(infections = sum(infections),
            ...
            IFR = all_deaths / infections) %>%
  group_by(age_groups) %>%
  summarise(IFR = list(summarise_mcmc(IFR, q = c((0:100)/100) ))) %>% #Add all percentiles
  unnest(cols = c(IFR))


write.csv(ifr_age,"../China_p_death_by_age_range.csv")
...
proportion_infections_severe <- predicted %>%
  ...
  group_by(age_groups) %>%
  summarise(hosp = list(summarise_mcmc(hosp, q = c((0:100)/100) ))) %>%
  unnest(col = c(hosp))
...
write.csv(proportion_infections_severe,"../China_p_severe_by_age_range.csv")
