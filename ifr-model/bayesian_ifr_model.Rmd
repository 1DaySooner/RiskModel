...
```{r open-safely-age, fig.width = 3, fig.height = 3, fig.cap = "Fatality rates as a function of age in OpenSAFELY data."}
como_df %>%
...
* Reduction possible from only analysing healthy population (defined as lack of co-morbidities listed above) is `r mean(alpha_exp)` with 95% interval from `r lci(alpha_exp)` to `r uci(alpha_exp)`. 
    + This estimate is obtained under a strong assumption that infection rates in general population are the same as in the healthy sub-population.
    + Our `r mean(alpha_exp)` estimate is much higher than the mean "crude" risk ratio of 1.53 which we would obtain by looking at 20-29 age group only, but is consistent with that (very uncertain) estimate.
