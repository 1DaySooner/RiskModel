...
What follows is an adaptation of typical methods of Bayesian evidence synthesis to analysis of IFRs. IFR is the ratio of deaths to infections in a given population. Early estimates of Covid-19 mortality risk, e.g. by @verity_estimates_2020, placed it at over 0.6% globally. However, it's also evident from data that IFR is orders of magnitude higher in particular high risk groups, especially in the elderly, than in the general population. Such relationship seems to be consistent across different countries.
...
where, for $k$-th observation, $\text{age}_k$ is the age group, with each age group given different "baseline" fatality rates, and $\text{comorb}_k$ and $\text{male}_k$ are indicator variables.
...
* Reduction possible from only analysing healthy population (defined as lack of co-morbidities listed above) is `r mean(alpha_exp)` with 95% interval from `r lci(alpha_exp)` to `r uci(alpha_exp)`. This estimate is obtained under a strong assumption that infection rates in general population are the same as in the healthy sub-population.
