---
title: "Bayesian evidence synthesis on COVID-19 mortality risk in HCT volunteers"
...
```{r open-safely-age, fig.width = 5, fig.height = 3, fig.cap = "Fatality rates as a function of age in OpenSAFELY data."}
open_safely_df_long %>%
  ...
  geom_point() +
  geom_line() +
    xlab("Age") + ylab("log(fatality rate)") +
  theme_minimal() +
...
```{r open-safely-ppc, fig.width = 8, fig.height = 7, fig.cap = "Comparison of posterior predictive numbers of deaths from the fitted Bayesian model (mean and 95% intervals) with data inputs (circles). For each age grouping we have 4 estimates: male/female and healthy (`No.Comorbidities` equal to 1) vs general population (0)"}
open_safely_df_long %>%
  filter(group != "all") %>% 
...
* In healthy population (defined as lack of co-morbidities listed above), the average mortality risk is `r rr_df["20","mean"]` times lower than in the general population, with 95% uncertainty interval from `r rr_df["20", "lci"]` to `r rr_df["20", "uci"]`.
    + Our `r rr_df["20","mean"]` estimate is a bit higher than the mean "crude" risk ratio of 1.53 because we use Bayesian hierarchical model. 
...
```{r output-csv}
csv_df <- data.frame(replications, #replications for mean IFR in 20-30 (takes into account heterogeneity) 
...
write.csv(csv_df, file = "output/bayesian-posteriors-shiny.csv")
```
