...
save(fit, stan_data_archived, file = "output/main_stan_model_15feb2021.Rdata")
```

```{r}
load("output/main_stan_model_15feb2021.Rdata")
if(!identical(stan_data, stan_data_archived))
  stop("Input data into models does not match")
```
...
# Summary of included studies and complete input data


We present two tables, one listing all study-level information and another breaking down information by age group.
References to all included studies are given in a separate bibliography at the end of this appendix. Below the table, we provide a list that can be used to cross-reference study locations with their bibliographic references.


```{r input-data-studies}
df_tab <- all_data_xls %>% 
  arrange(Median_Age, Study) %>%
  select(Study, StudyID, StudyType, EndDate, 
         AgeGroup, 
         Deaths, 
...
df_tab_joint <- df_tab %>% group_by(Study, EndDate) %>% 
  summarise(age = paste(unique(AgeGroup), collapse = ", "), 
            deaths = sum(Deaths), 
            population = sum(Population), 
            IFR = sum(InfectionRate*Population)/sum(Population),
            .groups = "keep")

study_xref <- all_data_xls %>% select(Study, StudyID) %>% distinct() %>% arrange(Study)

write.csv(df_tab, file = '../ShinyApp/source_data.csv') # For the app.
write.csv(df_tab_joint, file = '../ShinyApp/source_data_studies.csv') # For the app.
  
df_tab_joint %>%
  kbl(longtable = TRUE, booktabs = TRUE, 
      caption = "Complete table of studies used by the meta-analysis model.",
      col.names = c("Study", "End date", "Age groups", 
                    "Deaths", "N", "IR per 100"))%>%
...
`r paste("", paste0(study_xref$Study, ": ", paste0("@", study_xref$StudyID)), collapse = "; ")`

```{r  input-data-big}

df_tab %>%
  mutate(StudyID = paste0("@", StudyID)) %>%
  select(-StudyType, -EndDate, -StudyID) %>%
  kbl(longtable = TRUE, booktabs = TRUE, 
      caption = "Complete table of inputs used by the meta-analysis model and crude IFR's.",
      col.names = c("Study location", "Age", 
                    "Deaths", "N", "Mean", "2.5%", "97.5%", "IFR/100k"))%>%
  kable_styling(latex_options = c("repeat_header")) %>%
  add_header_above(c(" " = 4, "Infection rate (%)" = 3, " " = 1))
```
