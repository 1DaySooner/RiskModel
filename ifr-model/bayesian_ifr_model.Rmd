...
author: "Witold Więcek for 1 Day Sooner"
...
__WW: I WILL WORK ON THIS IN NOV-DEC 2020. ALL 4 OF THESE ARE ESSENTIAL__
...
output_df <- data.frame(
  ...
  mutate(
    ...
    hyperifr_25     = inv_logit(hypermean),
    hyperifr_30     = inv_logit(hypermean + .5*beta),
    hyperifr_35     = inv_logit(hypermean + beta),
  ...
  mutate(
    final_ifr_25 = ifr_min_25/rr_healthy_25,
    final_ifr_30 = ifr_min_30/rr_healthy_30,
    final_ifr_35 = ifr_min_35/rr_healthy_35
  )
...
In conclusion, the implications of the model for risk in healthy young subjects are as follows:

* We find that average IFR in 20-29 age group for the studies included in this analysis is `r mean(hyperifr_25)` with 95% interval from `r lci(hyperifr_25)` to `r uci(hyperifr_25)`.
    ...
    + Extending the HCT population to also include 30-39 year olds would lead to mean IFR of `r mean(hyperifr_30)` with 95% interval from `r lci(hyperifr_30)` to `r uci(hyperifr_30)`. Lowest mean IFR would then be `r mean(ifr_min_30)` (also in `r names(theta_mean)[which.min(theta_mean)]`).
* In healthy population (defined as lack of co-morbidities listed above), the average mortality risk in 20-29 year olds is `r mean(rr_healthy_25)` times lower than in the general population, with 95% uncertainty interval from `r lci(rr_healthy_25)` to `r uci(rr_healthy_25)`.
    + Our `r mean(rr_healthy_25)` estimate is a bit higher than the mean "crude" risk ratio of 1.53 because we use Bayesian hierarchical model. 
...
* Combining the smallest posterior IFR for 20-29 year olds with our estimated fold-reduction due to excluding individuals with co-morbidities from the population would lead to a mean infection fatality risk of  `r mean(final_ifr_25)` with 95% Bayesian interval from `r lci(final_ifr_25)` to `r uci(final_ifr_25)`.
    + In 20-39 year old subjects the risk would be `r mean(final_ifr_30)` (`r lci(final_ifr_30)` to `r uci(final_ifr_30)`).
...
```{r open-safely-all}
stan_df %>%
  group_by(Age_min, no_com, Gender) %>%
  summarise(p=sum(Population), f=sum(f), .groups = "keep") %>%
  mutate(m = f/p) %>%
  mutate(m = 1e05*m) %>%
  mutate(Age_min = paste(Age_min, "to", ifelse(Age_min == 70, "Inf", Age_min+9))) %>%
  ...
  kbl(col.names = c("Age group", "Gender", "All", "Healthy", "All", "Healthy", "All", "Healthy", "RR"),
      ...
      format.args = list(big.mark = ",")) %>%
  kable_styling() %>%
  add_header_above(c(" " = 2, "Population" = 2, "Fatalities" = 2, "FR per 100k" = 2, " " = 1)) %>%
